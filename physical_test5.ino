...
// Wheel distance (adjust as per your robot’s actual physical value)
float wheelDistance = 0.2;  // in meters, for example
...
float KpLeft = 1.0, KiLeft = 0.0, KdLeft = 0.0;
float KpRight = 1.0, KiRight = 0.0, KdRight = 0.0;

// For derivative term
float leftErrorPrevious = 0;
float rightErrorPrevious = 0;

// Output from PID controller
float speedSetLeft = 0;
float speedSetRight = 0;
...
void setup() {
  ...
  OCR1A = 1562;                        // Compare match value for 100ms at 16MHz with 1024 prescaler
...
void loop() {
  ...
  if (robotRunning) {
    ...
    float robotOmega = (rightSpeedActual - leftSpeedActual) / wheel_base;
    ...
    desired_speed_motor1  = map(constrain(abs(speedSetLeft),  0, 0.18), 0, 0.18, 255, 0);  //This is a PWM value for left motor driver
    desired_speed_motor2 = map(constrain(abs(speedSetRight), 0, 0.18), 0, 0.18, 255, 0);

    analogWrite(MOTOR1_RV, desired_speed_motor1);
    analogWrite(MOTOR2_RV, desired_speed_motor2);
    Serial.print("  ");
    Serial.print(desired_speed_motor1);
    Serial.print("  ");
    Serial.println(desired_speed_motor2);
