...
volatile bool calculateRPMFlag = false;  // Flag set by Timer1 ISR every 100ms
volatile long Lcount = 0;  // Left encoder tick count
volatile long Rcount = 0;  // Right encoder tick count
...
void LeftmotorISR() {
  // On rising‐edge of A, read B. HIGH→+1, LOW→–1
